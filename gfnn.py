...
def get_parameters():
    parser = argparse.ArgumentParser(description='gfNN')
    ...
    parser.add_argument('--model', type=str, default='gfnn', help='graph neural network model')
...
def process_data(device, dataset, gso_type, K):
    ...
    if device == torch.device('cpu'):
        feature = utility.calc_sgc_feature(gso, feature, K, device)
...
def prepare_model(n_feat, n_hid, n_class, enable_bias, droprate, early_stopping_patience, model_save_path, opt, lr):
    model = models.gfNN(n_feat, n_hid, n_class, enable_bias, droprate).to(device)
...
if __name__ == "__main__":
    ...
    feature, filter, label, idx_train, idx_val, idx_test, n_feat, n_class = process_data(device, dataset, gso_type, K)
    model, loss, early_stopping, optimizer, scheduler = prepare_model(n_feat, n_hid, n_class, enable_bias, droprate, early_stopping_patience, model_save_path, opt, lr)
